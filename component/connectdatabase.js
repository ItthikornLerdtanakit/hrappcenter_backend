...
const connectdatabase_department = mysql.createPool({ host: process.env.DB_HOST, user: process.env.DB_USER, password: process.env.DB_PASS, database: process.env.DB_DEPARTMENT, timezone: process.env.DB_TIMEZONE });

const testDB = async () => {
    try {
        await connectdatabase.query('SELECT 1');
        console.log('Connection Success | DB: Employee');
        await connectdatabase_department.query('SELECT 1');
        console.log('Connection Success | DB: Department');
    } catch (err) {
